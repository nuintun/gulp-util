...
module.exports = function(vinyl, options, done, cache, debug) {
  if (options.cache) {
    ...
    if (cached === null) {
      ...
      // debug
      debug('read: %s from cache', colors.magenta(util.pathFromCwd(vinyl.path)));
...
function parse(vinyl, options, done, debug) {
  ...
  // debug
  debug('use plugin: %s', colors.green(name));
  // debug
  debug('read: %s', colors.magenta(util.pathFromCwd(vinyl.path)));
